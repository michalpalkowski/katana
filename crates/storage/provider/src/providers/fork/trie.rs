...
impl<Db: Database> TrieWriter for ForkedProvider<Db> {
    ...
    fn compute_state_root(
        ...
        // Use std::thread::spawn to avoid multi-threaded runtime requirement
        let result: Result<GetStorageProofResponse, ProviderError> = {
            let fork_url = self.fork_url.clone();
            let state_updates_clone = state_updates.clone();

            std::thread::spawn(move || -> Result<GetStorageProofResponse, ProviderError> {
                let rt = tokio::runtime::Runtime::new().map_err(|e| {
                    ProviderError::ParsingError(format!("Failed to create runtime: {}", e))
                })?;

                rt.block_on(async move {
                    // Create new HTTP client for this operation
                    ...
                    let response: GetStorageProofResponse = client
                        .request(
                            ...
                            rpc_params![
                                ...
                                if contracts_storage_keys.is_empty() {
                                    None
                                } else {
            ...
            .join()
            .map_err(|_| ProviderError::ParsingError("Thread panicked".to_string()))?
        };
